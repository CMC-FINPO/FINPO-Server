        ...
        try {
            ...
            if (dto.interestCategories() != null) {
                dto.interestCategories().forEach(interestCategoryDto -> {
                    InterestCategory interestCategory = interestCategoryRepository.findById(interestCategoryDto.id())
                        .get();
    ...
    public Page<NotificationDto> getMyHistories(Long lastId, Pageable pageable) {
        try {
            if (Optional.ofNullable(lastId).isPresent()) {
                return notificationRepository.findByUserId(SecurityUtil.getCurrentUserId(), lastId, pageable)
                    .map(NotificationDto::response);
            }
            return notificationRepository.findByUserId(SecurityUtil.getCurrentUserId(), pageable)
                .map(NotificationDto::response);
        } catch (GeneralException e) {
            throw e;
        } catch (Exception e) {
