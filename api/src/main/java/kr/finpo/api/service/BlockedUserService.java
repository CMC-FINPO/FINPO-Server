...
public class BlockedUserService {
    ...
    public List<BlockedUserDto> getNonAnonymities() {
        try {
            return blockedUserRepository.findByUserIdAndAnonymityOrderByIdDesc(SecurityUtil.getCurrentUserId(), false)
                .stream().map(BlockedUserDto::response).toList();
    ...
    public Boolean insert(Long postId, Long commentId) {
        try {
            ...
            if (blockedUserRepository.findOneByUserIdAndBlockedUserIdAndAnonymity(SecurityUtil.getCurrentUserId(),
                blockUser.getId(), anonymity).isPresent()) {
