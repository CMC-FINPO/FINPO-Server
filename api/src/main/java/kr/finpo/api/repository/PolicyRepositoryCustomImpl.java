...
public class PolicyRepositoryCustomImpl implements PolicyRepositoryCustom {
    ...
    @Override
    public Page<Policy> querydslFindMy(List<InterestCategoryDto> myCategoryDtos, List<InterestRegionDto> myRegionDtos,
        Pageable pageable) {
    ...
    @Override
    public Page<Policy> querydslFindbyTitle(String title, LocalDate startDate, LocalDate endDate,
        List<Long> categoryIds, List<Long> regionIds, Boolean status, Pageable pageable) {
        ...
        if (!isEmpty(startDate)) {
            builder.and(p.startDate.after(startDate.minusDays(1)));
        }
        if (!isEmpty(endDate)) {
            builder.and(p.endDate.before(startDate.plusDays(1)));
        }
        if (!isEmpty(status)) {
