...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    Long countComments(Long id, Long userId);
    ...
    Page<Post> findAll(Long userId, Long lastId, String content, Pageable pageable);

    @Query("SELECT po FROM Post po WHERE po.user.id = :userId AND po.status = true")
    Page<Post> findMy(Long userId, Pageable pageable);
