...
public record CommentDto(
    ...
    public static CommentDto response(Comment comment, Boolean showPost, List<Comment> childs) {
        if (!comment.getUser().getStatus()) {
            return withdrawResponse(comment, showPost, childs);
        }
        if (!comment.getStatus()) {
            return deletedResponse(comment, showPost, childs);
        }
        ...
        return new CommentDto(
            ...
            comment.getUser().getId()
                .equals(Optional.ofNullable(comment.getPost().getUser()).map(User::getId).orElse(null)),
            ...
            childs == null || childs.isEmpty() ? null
                : childs.stream().map(child -> CommentDto.response(child, false, null)).toList()
        );
    }
